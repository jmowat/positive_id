...
export class BrowseFormComponent implements OnInit {
  ...
  getVehiclesBySelectedPlatform() {
    const constraint = { type: this.platforms.selectedOption.id };
  ...
  changePlatform() {
    ...
    this.sides = SelectBoxFactory
      .createSelectBoxOptions('Any Side', FilterHelper
        .getSides(FilterHelper.filter(this.getConstraints(), this.vehicles)));
    this.eras = SelectBoxFactory
      .createSelectBoxOptions('Any Era', FilterHelper
        .getEras(FilterHelper.filter(this.getConstraints(), this.vehicles)));
  }
