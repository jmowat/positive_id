...
describe('Quiz', () => {
  ...
  describe('ground vehicle quizzes', () => {
    ...
    it('should be able to concat five and luchs to make a new array', () => {
      const luchsArray = FIVE_VEHICLES.concat(LUCHS);
      expect(luchsArray).toBeTruthy();
    });
  ...
  describe('quiz question', () => {
    ...
    it('should randomly choose which image to show', () => {
      ...
      const question = new QuizQuestion(card, [], numToShow);
    });

    it('should allow a specific image to show', () => {
      const card = new VehicleCard(BMP2[0], {});
      const question = new QuizQuestion(card, [], numToShow);
      const img = question.getImagePath({
        size: 'large',
        perspective: 'side',
        distance: 'near'
      });
    ...
    it('should remember which image it chose to show', () => {
      const card = new VehicleCard(BMP2[0], {});
      const question = new QuizQuestion(card, [], numToShow);
      const img = question.getImagePath(undefined);
      for (let i = 0; i < 10; i++) {
    ...
    it('should display a list of x possible answers with the right answer being first', () => {
      ...
      const question = new QuizQuestion(card, ['a', 'b', 'c', 'd', 'e', 'f', 'g'], 5, randomize);
    ...
    it('should not have duplicates of itself', () => {
      const card = new VehicleCard(BMP2[0], {});
      const randomize = false;
      const question = new QuizQuestion(card, ['BMP-2', 'b', 'c', 'd', 'e'], 5, randomize);
    ...
    it('should display a list of x possible answers with the right answer being shuffled in', () => {
      ...
      const question = new QuizQuestion(card, ['BMP-2', 'b', 'c', 'd', 'e', 'f', 'g'], 5, randomize);
